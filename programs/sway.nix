{pkgs, lib, mkLiteral, ... }:
{
  ...
  config = {
    input = {
      "type:keyboard" = {
        xkb_file = "/home/natalie/keyboard/config/wayland_programmerdvorak.xkb";
      };
    };
    ...
    let 
      term = "kitty";
      menu = "rofi -modes \"drun,run\" -show drun";
      mod  = "Mod4";
    in 
    lib.mkDefault {
      "${mod}+Shift+c"     = "exec swaymsg restart";
      "${mod}+Shift+e"     = "exec sway-nagbar -t warning -m 'exit?' -B 'Yes' 'swaymsg exit'";
  ...
  startup = [
    ...
    { command = "swaybg -i ~/Pictures/wallpaper"; always = true; }
