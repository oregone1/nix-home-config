...
if awesome.startup_errors then
    ...
    logFile = io.open(string.format("%s/.config/awesome/awesome.log", "w"), os.getenv("HOME"))
    logFile:write(os.date("%Y-%m-%d-%H:%M:%S"), "Startup Error:", '\n', awesome.startup_errors, '\n')
    logfile.close()
end
...
do
    ...
    awesome.connect_signal("debug::error", function (err)
        ...
        logFile = io.open(string.format("%s/.config/awesome/awesome.log", "w"), os.getenv("HOME"))
        logFile:write(os.date("%Y-%m-%d-%H:%M:%S"), "Runtime Error:", '\n', tostring(err), '\n')
        logfile.close()
        in_error = false
    end)
end
...
beautiful.init(string.format("%s/.config/awesome/themes/catppuccin/theme.lua", os.getenv("HOME")))

-- modkey or mfind vertical asymptote of (3z^2-3)/(2z^2+7z+5)od4 = super key
local modkey       = "Mod4"
local altkey       = "Mod1"
